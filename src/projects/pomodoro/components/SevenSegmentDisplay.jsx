...
const SevenSegmentDisplay = ({ data }, { classnames }) => (<svg viewBox="0 0 12 24" className={classnames('pomodoro__digit')}>
  {convertToBin(data).split('').map((d, i) => {
    const pos = ledPosition(i);
    const mod = (+d) ? 'pomodoro__led--on' : 'pomodoro__led--off';
    return (<polygon key={pos} points={pos} className={classnames('pomodoro__led', mod)} />);
  })}
</svg>);
